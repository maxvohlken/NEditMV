...
/* Forcibly disable the XmIm...() stuff. */
#undef HAVE_XMIMREGISTER
...
static void extendEndAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void dragOrGrabFocusAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void dragOrExtendAdjustAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void dragOrExtendEndAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
...
static void secondaryOrDragAdjustAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static Boolean blockDragStart(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static Boolean blockDragAdjust(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static Boolean blockDragEnd(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
...
static void cutClipboardAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void secondaryOrClipboardAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void copySecondaryAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void moveSecondaryAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
...
static void selectSecondaryWord(Widget w, int pos);
static void selectLine(Widget w, int pos);
static void selectSecondaryLine(Widget w, int pos);
...
static void selectGroup(Widget w, int pos);
static void selectSecondaryGroup(Widget w, int pos);
static int startOfGroup(TextWidget w, int pos);
static int endOfGroup(TextWidget w, int pos);
...
static int wrapLine(TextWidget tw, textBuffer *buf, int bufOffset,
    	int lineStartPos, int lineEndPos, int limitPos, int wrapMargin,
    	int *breakAt, int *charsAdded);
...
#ifdef _WIN32
"\
		   Ctrl	<KeyPress>x: cut_clipboard()\n\
		   Ctrl	<KeyPress>c: copy_clipboard()\n\
		   Ctrl	<KeyPress>v: paste_clipboard()\n\
"
#else
"\
	 Shift Ctrl	<KeyRelease>osfCut: cut_primary()\n\
				<KeyRelease>osfCut: cut_clipboard()\n\
				<KeyRelease>osfCopy: copy_clipboard()\n\
				<KeyRelease>osfPaste: paste_clipboard()\n\
"
#endif
"\
				<KeyRelease>osfPrimaryPaste: copy_primary()\n\
...
~Alt~Meta  Ctrl	<KeyPress>v: paste_clipboard()\n\
~Alt~Meta  Ctrl	<KeyPress>c: copy_clipboard()\n\
~Alt~Meta  Ctrl	<KeyPress>x: cut_clipboard()\n\
~Alt~Meta  Ctrl	<KeyPress>u: delete_to_start_of_line()\n\
	~Shift Ctrl	<KeyPress>Return: newline_and_indent()\n\
	 Shift~Ctrl	<KeyPress>Return: newline_no_indent()\n\
	~Shift~Ctrl	<KeyPress>Return: newline()\n\
...
~Meta~Alt Shift Ctrl<KeyPress>space: key_select()\n\
~Alt~Meta  Ctrl	<KeyPress>slash: select_all()\n\
~Alt~Meta  Ctrl	<KeyPress>backslash: deselect_all()\n\
...
Meta       Ctrl	<Btn1Down>: move_destination()\n\
     ...
     Shift		<Btn1Down>: extend_start()\n\
				<Btn1Down>: grab_focus()\n\
	       Ctrl Button1<MotionNotify>: extend_adjust(rect)\n\
	      ~Ctrl Button1<MotionNotify>: extend_adjust()\n\
				<Btn1Up>: extend_end()\n\
...
~Alt~Meta  Ctrl <Btn3Down>: mouse_pan()\n\
~Alt~Meta  Ctrl Button3<MotionNotify>: mouse_pan()\n\
                <Btn3Up>: end_drag()\n\
				<FocusIn>: focusIn()\n\
				<FocusOut>: focusOut()\n\
";
...
static XtActionsRec actionsList[] = {
    ...
    {"drag-or-grab-focus", dragOrGrabFocusAP},
    {"drag_or_grab_focus", dragOrGrabFocusAP},
    {"drag-or-extend-adjust", dragOrExtendAdjustAP},
    {"drag_or_extend_adjust", dragOrExtendAdjustAP},
    {"drag-or-extend-end", dragOrExtendEndAP},
    {"drag_or_extend_end", dragOrExtendEndAP},
    ...
    {"end-drag", endDragAP},
    {"end_drag", endDragAP},
    {"move-secondary", moveSecondaryAP},
    {"move_secondary", moveSecondaryAP},
    {"copy-to", copyToAP},
    {"copy_to", copyToAP},
    {"copy-to-or-end-drag", copyToOrEndDragAP},
    {"copy_to_or_end_drag", copyToOrEndDragAP},
    {"copy-secondary", copySecondaryAP},
    {"copy_secondary", copySecondaryAP},
    {"exchange", exchangeAP},
    {"secondary-or-clipboard", secondaryOrClipboardAP},
    {"secondary_or_clipboard", secondaryOrClipboardAP},
...
static XtResource resources[] = {
    ...
    {textNdragStartsOnCharacter, textCDragStartsOnCharacter, XmRBoolean, sizeof(Boolean),
      XtOffset(TextWidget, text.dragStartsOnCharacter), XmRString, "False"},
    {textNenableQuadrupleClick, textCEnableQuadrupleClick, XmRBoolean, sizeof(Boolean),
      XtOffset(TextWidget, text.enableQuadrupleClick), XmRString, "True"},
    ...
    {textNhScrollBar, textCHScrollBar, XmRWidget, sizeof(Widget),
      XtOffset(TextWidget, text.hScrollBar), XmRPointer, NULL},
    {textNvScrollBar, textCVScrollBar, XmRWidget, sizeof(Widget),
      XtOffset(TextWidget, text.vScrollBar), XmRPointer, NULL},
    ...
    {textNsmartIndentCallback, textCSmartIndentCallback, XmRCallback,
      ...
      NULL},
    {textNselectThreshold, textCSelectThreshold, XmRInt, sizeof(int),
      XtOffset(TextWidget, text.selectThreshold), XmRString, "5"},
    {textNverticalScrollDelay, textCVerticalScrollDelay, XmRInt, sizeof(int),
      XtOffset(TextWidget, text.verticalScrollDelay), XmRString, "50"},
};
...
static void initialize(TextWidget request, TextWidget new)
{
    ...
    new->text.textD = TextDCreate((Widget)new,
	    new->text.marginWidth, new->text.marginHeight,
    ...
    /* Record the scrollbar widgets created in TextDCreate() for calls
    ** to XtVaGetValues() for textNhScrollBar and textNvScrollBar. */
    new->text.hScrollBar = new->text.textD->hScrollBar;
    new->text.vScrollBar = new->text.textD->vScrollBar;
    
    /* Create a list of the delimiters that does not contain whitespace. */
    {
    char *s, *d;
    
    delimiters = XtMalloc(strlen(new->text.delimiters)+1);
    for(s = new->text.delimiters, d = delimiters; *s; s++) {
    	if(!(*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')) {
    		*d = *s;
    		d++;
    	}
    ...
    *d = 0;
    new->text.nonWhiteSpaceDelimiters = delimiters;
    }
    ...
    delimiters = XtMalloc(strlen(new->text.delimiters) + 5);
    sprintf(delimiters, "%s%s", " \t\n\r", new->text.delimiters);
    ...
    new->text.lastBtnDown = -1;
    new->text.lastBtnDownTime = 0;
    new->text.selectionOwner = False;
    new->text.motifDestOwner = False;
    new->text.emTabsBeforeCursor = 0;
    
    /* Setup selection handling */
    HandleXSelections((Widget)new);

#ifdef HAVE_XMIMREGISTER
...
static void destroy(TextWidget w)
{
    ...
    XtFree(w->text.nonWhiteSpaceDelimiters);
...
/*
** Set the contents of the widget to the contents of the character string
** string.
*/
void TextSetString(Widget w, char *string)
{
    BufSetAll(TextGetBuffer(w), string);
}
...
{
    return TextDGetLastInsertPosition(((TextWidget)w)->text.textD);
}
    	...
    	int allowPendingDelete, int allowWrap)
{
    ...
    /* Insert the text.  Where possible, use TextDInsert which is optimized
       for less redraw. */
    if (replaceSel) {
    	BufReplaceSelected(buf, wrappedText, False);
...
static void grabFocusAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    ...
    TextWidget tw = (TextWidget)w;
    textDisp *textD = tw->text.textD;
    Cardinal lastBtnDown = tw->text.lastBtnDown;
    Cardinal lastBtnDownTime = tw->text.lastBtnDownTime;
    int row, column;
    int secondary = (hasKey("secondary", args, nArgs) || hasKey("sec", args, nArgs));
    ...
    tw->text.dragState = secondary ? SECONDARY_CLICKED : PRIMARY_CLICKED;
    tw->text.lastBtnDown = e->button;
    tw->text.lastBtnDownTime = e->time;
    ...
    if (tw->text.multiClickState != NO_CLICKS) {
		if (e->button == lastBtnDown && e->time < lastBtnDownTime + XtGetMultiClickTime(XtDisplay(w))) {
			if (tw->text.multiClickState == ONE_CLICK) {
				if(secondary) {
    				tw->text.anchor = TextDXYToCharPosition(textD, tw->text.btnDownX, tw->text.btnDownY);
					selectSecondaryGroup(w, tw->text.anchor);
				} else {
    				tw->text.anchor = TextDXYToCharPosition(textD, tw->text.btnDownX, tw->text.btnDownY);
					selectGroup(w, tw->text.anchor);
					callCursorMovementCBs(w, event);
				}
				return;
			} else if (tw->text.multiClickState == TWO_CLICKS) {
				if(secondary) {
					selectSecondaryLine(w, tw->text.anchor);
				} else {
					selectLine(w, tw->text.anchor);
					callCursorMovementCBs(w, event);
				}
				return;
    	    } else if (tw->text.enableQuadrupleClick && tw->text.multiClickState == THREE_CLICKS) {
				if(secondary) {
					BufSecondarySelect(textD->buffer, 0, textD->buffer->length, CHAR_SELECT);
				} else {
					BufSelect(textD->buffer, 0, textD->buffer->length, CHAR_SELECT);
				}
				return;
    	    } else {
    ...
    if(!secondary) {
	    ...
	    tw->text.anchor = TextDXYToPosition(textD, e->x, e->y);
    }
	...
	Cardinal *nArgs)
{
    ...
    int rectDrag = (hasKey("rect", args, nArgs) || hasKey("rectangular", args, nArgs));
    int secondary = (hasKey("secondary", args, nArgs) || hasKey("sec", args, nArgs));

    /* Make sure the proper initialization was done on mouse down */
    if ((!secondary && (dragState != PRIMARY_DRAG && dragState != PRIMARY_CLICKED &&
    	    dragState != PRIMARY_RECT_DRAG)) ||
    	(secondary && (dragState != SECONDARY_DRAG && dragState != SECONDARY_CLICKED &&
    ...
    if ((!secondary && tw->text.dragState == PRIMARY_CLICKED)
       || (secondary && tw->text.dragState == SECONDARY_CLICKED)) {
    	if (abs(e->x - tw->text.btnDownX) > tw->text.selectThreshold ||
    	    	abs(e->y - tw->text.btnDownY) > tw->text.selectThreshold) {
    	    if(tw->text.dragStartsOnCharacter && (e->x > tw->text.btnDownX || e->y > tw->text.btnDownY)) {
	    	    tw->text.anchor = TextDXYToCharPosition(
	    	    	tw->text.textD, tw->text.btnDownX, tw->text.btnDownY);
	    	} else {
	    	   	tw->text.anchor = TextDXYToPosition(
	    	   		tw->text.textD, tw->text.btnDownX, tw->text.btnDownY);
	    	}
    	} else
    	    return;
    ...
    tw->text.dragState = !secondary && rectDrag ? PRIMARY_RECT_DRAG :
    	                 !secondary && !rectDrag ?  PRIMARY_DRAG :
    	                 secondary && rectDrag ? SECONDARY_RECT_DRAG :
    	                 SECONDARY_DRAG;
...
{    
    ...
    selType selType = CHAR_SELECT;
    int rectangular = (hasKey("rect", args, nArgs) || hasKey("rectangular", args, nArgs));
    int secondary = (hasKey("secondary", args, nArgs) || hasKey("sec", args, nArgs));
 	enum dragStates dragState;
    ...
    if (sel->selected) {
    	if (sel->rectangular) {
    	    ...
    	    anchor = BufCountForwardDispChars(buf, anchorLineStart, rectAnchor);
   			...
   			TextWidget tw = (TextWidget)w;
   			int startPos, endPos, countBefore, countAfter;
			if (sel->type == WORD_SELECT) {
    			newPos = TextDXYToCharPosition(textD, e->x, e->y);
			    startPos = startOfGroup(tw, newPos);
			    endPos = endOfGroup(tw, newPos);
			    countBefore = abs(sel->start - newPos);
			    countAfter = abs(sel->end - newPos);
			} else if (sel->type == LINE_SELECT) {
    			newPos = TextDXYToPosition(textD, e->x, e->y);
			    startPos = BufStartOfLine(buf, newPos);
			    endPos = BufEndOfLine(buf, newPos);
			    endPos = min(endPos+1, buf->length);
			    countBefore = BufCountLines(buf, min(sel->start, newPos), max(sel->start, newPos));
			    countAfter = BufCountLines(buf, min(sel->end, newPos), max(sel->end, newPos)) - 1;
			    if(newPos >= sel->start && newPos <= sel->end && (countBefore + countAfter) <= 2 && (countBefore + countAfter) >= 1) {
			    	if(newPos < ((TextWidget)w)->text.anchor) {
			    		countBefore = 0;
			    		countAfter = 1;
			    	} else {
			    		countBefore = 1;
    			...
    			newPos = TextDXYToPosition(textD, e->x, e->y);
    	    	/* Select the newline if we are past the end of the line. */
    	    	if(newPos > ((TextWidget)w)->text.anchor) {
    	    		int posX, posY;
    	    		if(TextDPositionToXY(textD, newPos, &posX, &posY)) {
    	    			if(e->x > posX) {
	    	    			newPos++;
	    	    		}
    	    		}
			...
			/* Extend/reduce the selection from the side of the selection
			** that the new position is closer based on displayed characters.
			** If in LINE_SELECT mode then base it on the number of lines.
			*/
    	...
    	newPos = TextDXYToPosition(textD, e->x, e->y);
    	anchor = secondary ? ((TextWidget)w)->text.anchor : TextDGetInsertPosition(textD);
    	anchorLineStart = BufStartOfLine(buf, anchor);
    	rectAnchor = BufCountDispChars(buf, anchorLineStart, anchor);
    }
    ...
    if (rectangular) {
    	if(secondary) {
			BufSecRectSelect(buf, BufStartOfLine(buf, min(anchor, newPos)),
    	...
    	BufSecondarySelect(buf, min(anchor, newPos), max(anchor, newPos), selType);
		dragState = SECONDARY_DRAG;
    } else {
    	BufSelect(buf, min(anchor, newPos), max(anchor, newPos), selType);
		dragState = PRIMARY_DRAG;
    }
    
    /* Never mind the motion threshold, go right to dragging since
       extend-start is unambiguously the start of a selection */
    ((TextWidget)w)->text.dragState = dragState;
	...
	Cardinal *nArgs)
{
    ...
    if(((secondary && tw->text.dragState == SECONDARY_CLICKED)
        || (!secondary && tw->text.dragState == PRIMARY_CLICKED)) &&
        e->button == tw->text.lastBtnDown)
...
static void dragOrGrabFocusAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
	/* If we can't start a block drag then just call grab-focus. */
    if(!blockDragStart(w, event, args, nArgs)) {
    	grabFocusAP(w, event, args, nArgs);
    }
}
...
static void dragOrExtendAdjustAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
	/* If we can't do a block drag adjust then just call extend-adjust. */
    if(!blockDragAdjust(w, event, args, nArgs)) {
    	extendAdjustAP(w, event, args, nArgs);
    }
}
...
static void dragOrExtendEndAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
	/* If we can't do a block drag end then just call extend-end. */
    if(!blockDragEnd(w, event, args, nArgs)) {
    	extendEndAP(w, event, args, nArgs);
    }
}
...
{    
    String *params;
    Cardinal argc = *nArgs + 1;
    params = (String*)XtMalloc(argc * sizeof(String));
    memcpy(params, args, (argc - 1) * sizeof(String));
    params[argc-1] = "secondary";
    grabFocusAP(w, event, params, &argc);
    XtFree((char*)params);
}
	...
	Cardinal *nArgs)
{
    ...
    memcpy(params, args, (argc - 1) * sizeof(String));
    params[argc-1] = "secondary";
    extendAdjustAP(w, event, params, &argc);
    XtFree((char*)params);
}
	...
	Cardinal *nArgs)
{
    ...
    /* See if this is the start of a drag operation. 
    ** If the widget is not read only and the multi-click time has expired
    ** and this isn't for the secondary selection and the click was inside the
    ** primary selection then this is a drag operations. */
    if(!tw->text.readOnly && 
    	!(e->button == lastBtnDown && e->time <= (lastBtnDownTime + XtGetMultiClickTime(XtDisplay(w)))) &&
    	!secondary && TextDInSelection(textD, e->x, e->y)) {
   		/* Record the site of the initial button press and the initial character
   		...
   		tw->text.lastBtnDown = e->button;
   		tw->text.lastBtnDownTime = e->time;
   		tw->text.dragState = CLICKED_IN_SELECTION;
   		
   		/* This should still be treated as the first click of a selection. */
		tw->text.multiClickState = ONE_CLICK;
		return TRUE;
    }
    return FALSE;
}
    	...
    	Cardinal *nArgs)
{
	...
	/* deselect the selection if we didn't leave the CLICKED_IN_SELECTION
	** state. */
	if (dragState == CLICKED_IN_SELECTION) {
    	BufUnselect(textD->buffer);
    ...
    else if(dragState == PRIMARY_BLOCK_DRAG) {
...
static void copyToAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    ...
    if (secondary->selected) {
	    ...
	    if (primary->selected && rectangular) {
    		...
    		BufReplaceSelected(buf, textToCopy, False);
    		TextDSetInsertPosition(textD, buf->cursorPosHint);
	    } else if (rectangular) {
    		insertPos = TextDGetInsertPosition(textD);
...
static void exchangeAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    ...
    BufReplaceSelected(buf, secText, False);
	...
	BufSelect(buf, newPrimaryStart, newPrimaryEnd, CHAR_SELECT);
...
static void pasteClipboardAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
    if ((hasKey("rect", args, nArgs) || hasKey("rectangular", args, nArgs)))
...
/*
** Perform the secondary operation specified by the first arg if
** there is a secondary selection otherwise perform the clipboard
** operation specified by the second arg.
*/
static void secondaryOrClipboardAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
	...
	if (((TextWidget)w)->text.textD->buffer->secondary.selected) {
		if(!strCaseCmp(args[0], "copy")) {
			copySecondaryAP(w, event, args, nArgs);
		}
		else if(!strCaseCmp(args[0], "move")) {
			moveSecondaryAP(w, event, args, nArgs);
		}
		else if(!strCaseCmp(args[0], "exchange")) {
			exchangeAP(w, event, args, nArgs);
		}
	} else {
		if(!strCaseCmp(args[1], "copy")) {
			copyClipboardAP(w, event, args, nArgs);
		}
		else if(!strCaseCmp(args[1], "paste")) {
			pasteClipboardAP(w, event, args, nArgs);
		}
		else if(!strCaseCmp(args[1], "cut")) {
			cutClipboardAP(w, event, args, nArgs);
		}
	}
}

/* Copy the secondary selection into the primary selection or
   into the insertion cursor */
static void copySecondaryAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
	XButtonEvent *e = (XButtonEvent *)event;
    TextWidget tw = (TextWidget)w;
	textDisp *textD = tw->text.textD;
...
/* Move the secondary selection in place of the primary selection or
   move to the insertion cursor */
static void moveSecondaryAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
	XButtonEvent *e = (XButtonEvent *)event;
    TextWidget tw = (TextWidget)w;
	textDisp *textD = tw->text.textD;
	...
	Cardinal *nArgs)
{
    ...
    if (((TextWidget)w)->text.overstrike) {
		...
		if (c == '\n') {
#if defined(_WIN32) && defined(WIN32_BINARY_FILE_MODE)
			if(BufGetCharacter(textD->buffer, insertPos - 2) == '\r') {
    		    BufRemove(textD->buffer, insertPos - 2, insertPos);
				TextDSetInsertPosition(textD, insertPos - 2);
			} else {
...
#if defined(_WIN32) && defined(WIN32_BINARY_FILE_MODE)
		if(BufGetCharacter(textD->buffer, insertPos - 1) == '\n' &&
		   BufGetCharacter(textD->buffer, insertPos - 2) == '\r') {
    		BufRemove(textD->buffer, insertPos - 2, insertPos);
		    TextDSetInsertPosition(textD, insertPos - 2);
		} else {
    	BufRemove(textD->buffer, insertPos - 1, insertPos);
...
#if defined(_WIN32) && defined(WIN32_BINARY_FILE_MODE)
	if(BufGetCharacter(textD->buffer, insertPos) == '\r' && 
	   BufGetCharacter(textD->buffer, insertPos+1) == '\n') {
		BufRemove(textD->buffer, insertPos , insertPos + 2);
	} else {
...
/* 
** Move the cursor to the end of the current or the next word.
*/
static void forwardWordAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
    ...
    /* Skip the first set of delimiters */
...
/* 
** Move the cursor to the beginning of the currrent or the previous word.
*/
static void backwardWordAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
	...
	Cardinal *nArgs)
{
    ...
    if (!TextDMoveUp(((TextWidget)w)->text.textD))
    	XBell(XtDisplay(w), 0);
    keyMoveExtendSelection(w, event, insertPos, (hasKey("rect", args, nArgs) || hasKey("rectangular", args, nArgs)));
    checkAutoShowInsertPos(w);
    callCursorMovementCBs(w, event);
}
	...
	Cardinal *nArgs)
{
    ...
    int insertPos = TextDGetInsertPosition(textD);
	int newInsertPos;
	int toggle = hasKey("toggle", args, nArgs);
	int firstNonWhite = hasKey("non-space", args, nArgs);
	
    cancelDrag(w);
    if(toggle || firstNonWhite) {
		int startOfLine;
		int firstNonWhitePos;
		
    	startOfLine = TextDStartOfLine(textD, insertPos);

    	/* Find the first non-whitespace character after the start of 
    	   the line. */
    	for(firstNonWhitePos = startOfLine; startOfLine < textD->buffer->length; firstNonWhitePos++) {
    		char ch = BufGetCharacter(textD->buffer, firstNonWhitePos);
    		if(!(ch == ' ' || ch == '\t'))
    			break;
    	}

    	/* Toggle the insert position between the start of the line and the
    	   first non-whitespace character */
    	if(toggle) {
			if(firstNonWhite) {
    			newInsertPos = (insertPos == firstNonWhitePos) ? startOfLine : firstNonWhitePos;
    		}
    		else {
    			newInsertPos = (insertPos == startOfLine) ? firstNonWhitePos : startOfLine;
    		}
    	} else {
    		newInsertPos = firstNonWhitePos;
    	} 
    }
    else {
    	newInsertPos = TextDStartOfLine(textD, insertPos);
    }
    TextDSetInsertPosition(textD, newInsertPos);
    checkMoveSelectionChange(w, event, insertPos, args, nArgs);
    checkAutoShowInsertPos(w);
    callCursorMovementCBs(w, event);
}
...
static void endOfLineAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    ...
    int newInsertPos;
	int toggle = hasKey("toggle", args, nArgs);
	int lastNonWhite = hasKey("non-space", args, nArgs);

    cancelDrag(w);
    if(toggle || lastNonWhite) {
    	int endOfLine;
    	int lastNonWhitePos;
    	
    	endOfLine = TextDEndOfLine(textD, insertPos, False);

    	/* Find the last non-whitespace character before the end of 
    	   the line. */
    	for(lastNonWhitePos = endOfLine; lastNonWhitePos > 0; lastNonWhitePos--) {
    		char ch;
    		ch = BufGetCharacter(textD->buffer, lastNonWhitePos - 1);
    		if(!(ch == ' ' || ch == '\t'))
    			break;
    	}

    	/* Toggle the insert position between the end of the line and the
    	   last non-whitespace character */
    	if(toggle) {
			if(lastNonWhite) {
    			newInsertPos = (insertPos == lastNonWhitePos) ? endOfLine : lastNonWhitePos;
    		}
    		else {
    			newInsertPos = (insertPos == endOfLine) ? lastNonWhitePos : endOfLine;
    		}
    	} else {
    		newInsertPos = lastNonWhitePos;
    	}
    }
    else {
    	newInsertPos = TextDEndOfLine(textD, insertPos, False);
    }
    TextDSetInsertPosition(textD, newInsertPos);
    checkMoveSelectionChange(w, event, insertPos, args, nArgs);
    checkAutoShowInsertPos(w);
	...
	Cardinal *nArgs)
{
    ...
    cancelDrag(w);
    BufSelect(buf, 0, buf->length, CHAR_SELECT);
}
	...
	String *args, Cardinal *nArgs)
{
    if (hasKey("extend", args, nArgs))
    	keyMoveExtendSelection(w, event, startPos, (hasKey("rect", args, nArgs) || hasKey("rectangular", args, nArgs)));
    else
    	BufUnselect((((TextWidget)w)->text.textD)->buffer);
}
	...
	int rectangular)
{
	...
	BufRectSelect(buf, startPos, endPos, startCol, endCol);
    	...
    	if (abs(origPos - startPos) < abs(origPos - endPos))
    	    ...
    	    anchor = startPos;
    	BufSelect(buf, anchor, newPos, CHAR_SELECT);
    	...
    	int allowPendingDelete)
{
    ...
    if (allowPendingDelete && pendingSelection(w)) {
    	BufReplaceSelected(buf, chars, False);
    	TextDSetInsertPosition(textD, buf->cursorPosHint);
...
static void selectWord(Widget w, int pointerX)
{
    ...
    textBuffer *buf = textD->buffer;
    int insertPos = TextDGetInsertPosition(((TextWidget)w)->text.textD);
    int startPos, endPos, leftOf, x, y;
    
    TextPosToXY(w, insertPos, &x, &y);
    leftOf = pointerX < x;
    if ((leftOf && (insertPos == 0 || whiteSpaceAtPos(buf, insertPos-1))) ||
	    (!leftOf && (insertPos == buf->length ||
	      whiteSpaceAtPos(buf, insertPos)))) {
	for (startPos = insertPos; startPos > 0; startPos--)
	    if (!whiteSpaceAtPos(buf, startPos-1))
		break;
	for (endPos = insertPos; endPos < buf->length; endPos++)
	    if (!whiteSpaceAtPos(buf, endPos))
		break;
    } else {
	startPos = startOfWord(tw, insertPos);
	endPos = endOfWord(tw, insertPos);
    }
    BufSelect(textD->buffer, startPos, endPos, WORD_SELECT);
...
static void selectSecondaryWord(Widget w, int pos)
{
    ...
    startPos = startOfWord((TextWidget)w, pos);
    endPos = endOfWord((TextWidget)w, pos);
    BufSecondarySelect(((TextWidget)w)->text.textD->buffer, startPos, endPos, WORD_SELECT);
}
...
/*
** Select the group of like chars around the cursor. Like characters
...
static void selectGroup(Widget w, int pos)
{
    ...
    startPos = startOfGroup((TextWidget)w, pos);
    endPos = endOfGroup((TextWidget)w, pos);
   	insertPos = pos < (startPos + (endPos - startPos) / 2) ? startPos : endPos;
    BufSelect(((TextWidget)w)->text.textD->buffer, startPos, endPos, WORD_SELECT);
    TextDSetInsertPosition(((TextWidget)w)->text.textD, insertPos);
}
...
/*
** Returns the position of the start of a group of like characters.  Like
** characters are newline, space or tab, non-delimiter characters and the
** delimiter characters minus the newline, space and tab.
*/
static int startOfGroup(TextWidget w, int pos)
{
    textBuffer *buf = w->text.textD->buffer;
    ...
    int isWhiteSpace = 0;
    int isDelimiter = 0;
    char *searchChars = whiteSpace;
    
    /* return the start of the buffer if pos was out of range */
    if(pos < 0) {
    	return 0;
    }

    /* return the end of the buffer if pos was out of range */
    if(pos >= buf->length) {
    	return buf->length;
    }
    ...
    /* A newline is considered a word so just return its position. */
    if(c == '\n') {
    	return pos;
    }
    
    /* if the character at pos is a word, that is, it is not one of the
       delimiter characters, then return its start */
    if(strchr(w->text.delimiters, c) == NULL) {
    	if(BufSearchBackward(w->text.textD->buffer, pos, w->text.delimiters,
    	    &startPos))
    		return min(pos, startPos+1);
    	else
    ...
    /* If it is not a word then see if the character at pos is a 
       whitespace character. */
    if(strchr(whiteSpace, c) != NULL) {
    	isWhiteSpace = 1;
    	searchChars = whiteSpace;
    }
    /* If is not whitespace then see if it is a delimiter */
    else {
    	if(strchr(w->text.nonWhiteSpaceDelimiters, c) != NULL) {
   			isDelimiter = 1;
    		searchChars = w->text.nonWhiteSpaceDelimiters;
    	}
    }
    
    /* If pos was white space or a delimiter then return the start of
       the whitespace or the start of the group of delimiters */
    if(isWhiteSpace || isDelimiter) {
    	int foundPos;
    	startPos = pos;
    	while(BufSearchBackward(w->text.textD->buffer, startPos, searchChars,
    	    &foundPos)) {
    	    if(foundPos != startPos-1)
    	    	break;
    	    startPos = foundPos;
    	}
    }
    /* else return the original position. This shouldn't happen */
    else {
    		startPos = pos;
    }
...
/*
** Returns the position of the end of a group of like characters.  Like
** characters are newline, space or tab, non-delimiter characters and the
** delimiter characters minus the newline, space and tab.
*/
static int endOfGroup(TextWidget w, int pos)
{
    ...
    /* A newline is considered a word so just return its position + 1. */
    if(c == '\n') {
    	return pos + 1;
    }
    
    /* if the character at pos is a word, that is, it is not one of the
       delimiter characters, then return its end */
    if(strchr(w->text.delimiters, c) == NULL) {
    	if (!BufSearchForward(buf, pos, w->text.delimiters, &endPos))
    		endPos = buf->length;
		return endPos;
    ...
    /* If pos was white space or a delimiter then return the end of
       the whitespace or the end of the group of delimiters */
    if(isWhiteSpace || isDelimiter) {
    	int foundPos;
    	endPos = pos;
    	while(BufSearchForward(w->text.textD->buffer, endPos, searchChars, 
    		&foundPos)) {
    	    if(foundPos != endPos)
    	    	break;
    	    endPos = foundPos + 1;
    	}
    	endPos = min(endPos, buf->length);
    }
    /* else return the original position + 1. This shouldn't happen */
    else {
   		endPos = pos + 1;
    }
...
static void selectLine(Widget w, int pos)
{
    ...
    startPos = BufStartOfLine(buf, pos);
    endPos = BufEndOfLine(buf, pos);
    endPos = min(endPos+1, buf->length);
   	insertPos = pos <= (startPos + (endPos - startPos) / 2) ? startPos : endPos;
    BufSelect(buf, startPos, endPos, LINE_SELECT);
    TextDSetInsertPosition(((TextWidget)w)->text.textD, insertPos);
}
...
static void selectSecondaryLine(Widget w, int pos)
{
    ...
    endPos = BufEndOfLine(buf, pos);
    endPos = min(endPos+1, buf->length);
    BufSecondarySelect(buf, startPos, endPos, LINE_SELECT);
}
...
static void adjustSelection(TextWidget tw, int x, int y)
{
    textDisp *textD = tw->text.textD;
    textBuffer *buf = textD->buffer;
    int row, col, startCol = 0, endCol = 0, startPos = 0, endPos = 0;
    int newPos = 0;
    selType selType = CHAR_SELECT;
    
    if(tw->text.dragState == PRIMARY_RECT_DRAG || tw->text.dragState == PRIMARY_DRAG)
	    selType = BufGetSelectionType(buf);
	else if(tw->text.dragState == SECONDARY_RECT_DRAG || tw->text.dragState == SECONDARY_DRAG)
   	    selType = BufGetSecSelectType(buf);

    if (tw->text.dragState == PRIMARY_RECT_DRAG || tw->text.dragState == SECONDARY_RECT_DRAG) {
    	newPos = TextDXYToPosition(textD, x, y);
    ...
    } else if (tw->text.dragState == PRIMARY_DRAG || tw->text.dragState == SECONDARY_DRAG) {
		if (selType == WORD_SELECT) {
    		newPos = TextDXYToCharPosition(textD, x, y);
	    	startPos = startOfGroup(tw, min(tw->text.anchor, newPos));
	    	endPos = endOfGroup(tw, max(tw->text.anchor, newPos));
	    	newPos = newPos < (startPos + (endPos - startPos) / 2) ? startPos : endPos;
	    } else if (selType == LINE_SELECT) {
 			newPos = TextDXYToPosition(textD, x, y);
	        startPos = BufStartOfLine(buf, min(tw->text.anchor, newPos));
	        endPos = BufEndOfLine(buf, max(tw->text.anchor - 1, newPos));
	        endPos = min(endPos+1, buf->length);
	        newPos = newPos < tw->text.anchor ? startPos : endPos;
	    } else {
    	    newPos = TextDXYToPosition(textD, x, y);
    	    /* Select the newline if we are past the end of the line. */
    	    if(newPos > tw->text.anchor) {
    	    	int posX, posY;
    	    	if(TextDPositionToXY(textD, newPos, &posX, &posY)) {
    	    		if(x > posX) {
	    	    		newPos++;
    ...
    if (tw->text.dragState == PRIMARY_RECT_DRAG) {
		BufRectSelect(buf, startPos, endPos, startCol, endCol);
    } else if (tw->text.dragState == SECONDARY_RECT_DRAG) {
		BufSecRectSelect(buf, startPos, endPos, startCol, endCol);
    } else if (tw->text.dragState == PRIMARY_DRAG) {
		BufSelect(buf, startPos, endPos, selType);
    } else if (tw->text.dragState == SECONDARY_DRAG) {
		BufSecondarySelect(buf, startPos, endPos, selType);
    }

    /* Move the cursor */
    if (tw->text.dragState == PRIMARY_RECT_DRAG
     || tw->text.dragState == PRIMARY_DRAG) {
	    TextDSetInsertPosition(textD, newPos);
    ...
    while (pos < wrapBuf->length) {
    	...
    	if (c == '\n') {
    		...
    		if (!wrapLine(tw, wrapBuf, bufOffset, lineStartPos, pos,
    		    	limitPos, wrapMargin, &breakAt, &charsAdded)) {
    	    	    limitPos = pos;
    		} else {
    	...
    	int lineStartPos, int lineEndPos, int *length, int *column)
{
    ...
    indentPtr = indentStr = XtMalloc(indent + 3);
#if defined(_WIN32) && defined(WIN32_BINARY_FILE_MODE)
    *indentPtr++ = '\r';
#endif
    *indentPtr++ = '\n';
...
static void autoScrollTimerProc(XtPointer clientData, XtIntervalId *id)
{
    ...
    if (cursorX >= (int)w->core.width - w->text.marginWidth)
    	...
    	horizOffset -= fontWidth;
    newPos = w->text.mouseY - w->text.marginHeight;
    if (newPos >= (int)w->core.height)
    	topLineNum += ((newPos - (int)w->core.height) / fontHeight) + 1;
    else if (newPos < 0)
    	topLineNum += (newPos / fontHeight) - 1;
    ...
    w->text.autoScrollProcID = XtAppAddTimeOut(
    	    ...
    	    (w->text.verticalScrollDelay*fontWidth) / fontHeight :
    	    w->text.verticalScrollDelay, autoScrollTimerProc, w);
}
