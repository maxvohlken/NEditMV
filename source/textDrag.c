...
void BeginBlockDrag(TextWidget tw)
{
    ...
    if (sel->rectangular)
    	...
    	BufSelect(tw->text.dragOrigBuf, sel->start, sel->end, sel->type);
...
void BlockDragSelection(TextWidget tw, int x, int y, int dragType)
{
    ...
    if (rectangular || overlay) {
    	...
    	BufSelect(buf, insStart, insStart + origSel->end - origSel->start, origSel->type);
...
void CancelBlockDrag(TextWidget tw)
{
    ...
    if (origSel->rectangular)
    	...
    	BufSelect(buf, origSel->start, origSel->end, origSel->type);
