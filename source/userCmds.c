...
Select \"New\" to add a new command to the menu."),
    ...
    cmdLabel = XtVaCreateManagedWidget("cmdLabel", xmLabelGadgetClass, form,
    	    ...
    	    XmNleftAttachment, XmATTACH_POSITION,
    	    XmNleftPosition, LEFT_MARGIN_POS, 0);
    XmStringFree(s1);
	...
	int nMenuItems, int sensitive)
{
	...
	XtVaGetValues(items[n], XmNuserData, &userData, 0);
    	if (userData != PERMANENT_MENU_ITEM) {
	    	...
	    	XtVaGetValues(items[n], XmNsubMenuId, &subMenu, 0);
...
void SetBackgroundMenuUndoSensitivity(WindowInfo *window, int sensitive)
{
    ...
    for(w = window->editorInfo->master; w; w = w->nextSlave) {
    	if (w->bgMenuUndoItem != NULL)
    		XtSetSensitive(w->bgMenuUndoItem, sensitive);
	}
}
void SetBackgroundMenuRedoSensitivity(WindowInfo *window, int sensitive)
{
    WindowInfo *w;
    for(w = window->editorInfo->master; w; w = w->nextSlave) {
    	if (w->bgMenuRedoItem != NULL)
			XtSetSensitive(w->bgMenuRedoItem, sensitive);
	}
}
    	...
    	if (!strcmp(ShellMenuItems[i]->name, itemName)) {
    	    if ((ShellMenuItems[i]->output == TO_SAME_WINDOW ||
    	    	 (ShellMenuItems[i]->repInput && 
    	    	  ShellMenuItems[i]->input != FROM_NONE
    	    	 )
...
static void updateMenu(WindowInfo *window, int menuType)
{
    ...
    */
	...
	for (;;) {
		...
		/* Make it possible to have '>' in the menu label.
		** Use backslash to escape > */
		while(subSep != NULL && subSep > strippedName && subSep[-1] == '\\') {
			strcpy(subSep-1, subSep);
	    	subSep = strchr(subSep, '>');
		}
	    if (subSep == NULL) {
    ...
    DimSelectionDepUserMenuItems(window, window->editorInfo->buffer->primary.selected);
}
...
static void removeMenuItems(Widget menuPane)
{
    ...
    XtVaGetValues(menuPane, XmNchildren, &itemList, XmNnumChildren, &nItems,0);
	...
	XtVaGetValues(items[n], XmNuserData, &userData, 0);
    	if (userData != PERMANENT_MENU_ITEM) {
    	    if (XtClass(items[n]) == xmCascadeButtonWidgetClass) {
		XtVaGetValues(items[n], XmNsubMenuId, &subMenuID, 0);
		removeMenuItems(subMenuID);
#ifdef notdef	/* This is a memory and server resource leak (though both are
		   reclaimed on window closing).  In Motif 2.0 (and beyond?)
		   there is a potential crash during phase 2 widget destruction
		   in "SetCascadeField", and in Motif 1.2 there are free-memory
		   reads.  I would really like to be able to destroy this. */
	...
	int listType)
{
    ...
    for (i=0; i<nItems; i++) {
    	...
    	length += strlen(f->name) * 2; /* allow for \> expansions */
    ...
    *outPtr++ = '\n';
    for (i=0; i<nItems; i++) {
    	...
    	generateAcceleratorString(accStr, f->modifiers, f->keysym);
    	*outPtr++ = '\t';
    	for (c=f->name; *c!='\0'; c++) { /* Copy the menu name string, changing */
