...
void ShiftSelection(WindowInfo *window, int direction, int byTab)
{
    ...
    int shiftedLen, cursorPos, origLength, emTabDist, shiftDist;
	...
	if (selEnd < buf->length)
	    selEnd++;
	BufSelect(buf, selStart, selEnd, CHAR_SELECT);
    	isRect = False;
    ...
    if (byTab) {
    	...
    	shiftDist = 1;
    shiftedText = ShiftText(text, direction, 
    		byTab ? buf->useTabs : False, /* only use tabs when shifting by tabs */
    		buf->tabDist, shiftDist, &shiftedLen);
    XtFree(text);
    BufReplaceSelected(buf, shiftedText, True);
    XtFree(shiftedText);
}
	...
	int selStart, int selEnd, int rectStart, int rectEnd)
{
    ...
    textBuffer *tempBuf, *buf = window->editorInfo->buffer;
    char *text;
...
static void changeCase(WindowInfo *window, int makeUpper)
{
    ...
    if (!BufGetSelectionPos(buf, &start, &end, &isRect, &rectStart, &rectEnd)) {
    	...
    	*bufChar = makeUpper ? toupper(*bufChar) : tolower(*bufChar);
    	BufReplace(buf, cursorPos-1, cursorPos, bufChar);
    } else {
	text = BufGetSelectionText(buf);
	for (c=text; *c!='\0'; c++)
    	    *c = makeUpper ? toupper(*c) : tolower(*c);
	BufReplaceSelected(buf, text, True);
	XtFree(text);
    }
}
...
void FillSelection(WindowInfo *window)
{
    ...
    int hasSelection = window->editorInfo->buffer->primary.selected;
	...
	if (right != 0 && BufGetCharacter(buf, right-1) != '\n') {
    	...
    	BufSelect(buf, left, right, CHAR_SELECT);
    	text = BufGetRange(buf, left, right);
    }
    ...
    if (hasSelection) {
    	BufReplaceSelected(buf, filledText, True);
    } else {
