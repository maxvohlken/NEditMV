...
""\
	...
	re matching:\"<((m|q|qq)?/)(\\\\/|[^/])*(/[gimsox]?)>\":::Double Quoted String::\n\
	re match coloring:\"\\1\\4\":::Operators:re matching:C\n\
	re match escapes:\"\\\\(.|\\n)\":::Escaped Items:re matching:\n\
	re match variables:\"\\$[^a-zA-Z0-9_ /\\t\\n{]|\\$[0-9]+|[$@%][a-zA-Z_][a-zA-Z0-9_]*\":::Identifier:re matching:\n\
	re substitution:\"<((s|y|tr)/)(\\\\/|[^/])*(/)[^/]*(/[gimsox]?)\":::Double Quoted String::\n\
	re sub coloring:\"\\1\\4\\5\":::Operators:re substitution:C\n\
""\
...
""\
	...
	Comment:\"^[ \\t]*!\":\"$\"::Comment::\n\
	Comment Wrap:\"\\\\\\n\":::Comment:Comment:\n\
	Empty Resource Name:\"^[ \\t]*[^:]+[:]$\":::Identifier::\n\
	Resource:\"(^[ \\t]*[^:]+[:])\":\"$\"::String::\n\
	Resource Esc Chars:\"\\\\.\":::String1:Resource:\n\
	Resource Space Warning:\"[^ \\t]+[ \\t]+$\":::Flag:Resource:\n\
	Resource Name:\"\\1\":\"\"::Storage Type:Resource:C\n\
	Resource Wrap:\"\\\\\\n\":::String2:Resource:}",
     "NEdit Macro:1:0{\n\
	Comment:\"#\":\"$\"::Comment::\n\
...
void RenameStyle(char *oldStyle, char *newStyle)
{
    int i, j;
    
    for (i=0; i<NPatternSets; i++) {
    	for (j=0; j<PatternSets[i]->nPatterns; j++) {
    		if (!strcmp(oldStyle, PatternSets[i]->patterns[j].style)) {
				XtFree(PatternSets[i]->patterns[j].style);
				PatternSets[i]->patterns[j].style = CopyAllocatedString(newStyle);
			}
		}
	}
    if (HighlightDialog.shell != NULL) {
    	for (j=0; j<HighlightDialog.nPatterns; j++) {
    		if (!strcmp(oldStyle, HighlightDialog.patterns[j]->style)) {
				XtFree(HighlightDialog.patterns[j]->style);
				HighlightDialog.patterns[j]->style = CopyAllocatedString(newStyle);
			}
		}
    }
}
...
static int updateHSList(void)
{
    ...
    /* Check for style name changes. */
	for (i=0; i<NHighlightStyles; i++) {
		/* if the name has changed then update that items name in the
		** other data structures. */
		if(strcmp(HighlightStyles[i]->name, HSDialog.highlightStyleList[i]->name) != 0) {
			RenameStyle(HighlightStyles[i]->name, HSDialog.highlightStyleList[i]->name);
		}
	}
