...
textDisp *TextDCreate(Widget widget, 
	...
	int continuousWrap, int wrapMargin)
{
        ...
        visualPolicy == XmVARIABLE)
    {
    	...
    	textD->hScrollBar = XtVaCreateManagedWidget("textHorScrollBar",
...
{
    BufRemoveModifyCB(textD->buffer, bufModifiedCB, textD, True);
...
** character - unfinishedStyle) into fonts and colors; and a callback mechanism for
...
void TextDSetInsertPosition(textDisp *textD, int newPos)
{
    ...
    /* Don't allow the cursor to be positioned between a <cr><nl> pair. */
#ifdef _WIN32
	if(  BufGetCharacter(textD->buffer, newPos) == '\n'
	  && BufGetCharacter(textD->buffer, newPos-1) == '\r'
	  ) {
		newPos--;
	}
#endif
    ...
    textD->cursorLastPos = textD->cursorPos;
...
int TextDGetLastInsertPosition(textDisp *textD)
{
    return textD->cursorLastPos;
}
...
/*
** Translate window coordinates to the nearest character position.
*/
int TextDXYToCharPosition(textDisp *textD, int x, int y)
{
...
int TextDMoveRight(textDisp *textD)
{
    if (textD->cursorPos >= textD->buffer->length)
    	return False;
#ifdef _WIN32
    TextDSetInsertPosition(textD, textD->cursorPos + 
		((BufGetCharacter(textD->buffer, textD->cursorPos) == '\r' && 
		  BufGetCharacter(textD->buffer, textD->cursorPos + 1) == '\n') ? 2 : 1));
#else
...
#ifdef _WIN32
    TextDSetInsertPosition(textD, textD->cursorPos -  
		((BufGetCharacter(textD->buffer, textD->cursorPos - 1) == '\r' &&
		  BufGetCharacter(textD->buffer, textD->cursorPos - 2) == '\n') ? 2 : 1));
#else
    TextDSetInsertPosition(textD, textD->cursorPos - 1);
#endif
    return True;
...
#if 0
    /* Make sure the cursor is redisplayed */
    TextDRedisplayRange(textD, textD->cursorPos-1, textD->cursorPos+1);
#endif
}
	...
	char *string, int nChars)
{
    ...
    if (style & FILL_MASK) {
		if(style & SECONDARY_MASK) {
    	    XDrawLine(XtDisplay(textD->w), XtWindow(textD->w), textD->gc, x,
    	    	    y + textD->ascent, x + nChars * fs->max_bounds.width,
    	    	    y + fs->ascent);
		}
		else
    ...
    if (style & STYLE_LOOKUP_MASK) {
    	styleRec = &textD->styleTable[(style & STYLE_LOOKUP_MASK) - textD->unfinishedStyle];
    	fs = styleRec->font;
    	gc = textD->styleGC ;
	gcValues.font = fs->fid;
	gcValues.foreground = 
#ifndef IGNORE_SELECT_FOREGROUND_COLOR
			style&PRIMARY_MASK ? textD->selectFGPixel :
#endif
#ifndef IGNORE_HIGHLIGHT_FOREGROUND_COLOR
	    	style&HIGHLIGHT_MASK ? textD->highlightFGPixel : 
#endif
	    	styleRec->color;
...
static void drawCursor(textDisp *textD, int x, int y)
{
	...
	midY = y + fontHeight/2;
	nSegs = 0;
	segs[nSegs].x1 = left; segs[nSegs].y1 = y; segs[nSegs].x2 = x-1; segs[nSegs].y2 = y; nSegs++;
	segs[nSegs].x1 = x+1; segs[nSegs].y1 = y; segs[nSegs].x2 = right; segs[nSegs].y2 = y; nSegs++;
	segs[nSegs].x1 = x; segs[nSegs].y1 = y+1; segs[nSegs].x2 = x; segs[nSegs].y2 = y+2; nSegs++;
	segs[nSegs].x1 = x; segs[nSegs].y1 = midY; segs[nSegs].x2 = x; segs[nSegs].y2 = midY; nSegs++;
	segs[nSegs].x1 = x; segs[nSegs].y1 = bot-2; segs[nSegs].x2 = x; segs[nSegs].y2 = bot-1; nSegs++;
	segs[nSegs].x1 = left; segs[nSegs].y1 = bot; segs[nSegs].x2 = x-1; segs[nSegs].y2 = bot; nSegs++;
	segs[nSegs].x1 = x+1; segs[nSegs].y1 = bot; segs[nSegs].x2 = right; segs[nSegs].y2 = bot; nSegs++;
    	...
    	int lineLen, int lineIndex, int dispIndex)
{
    ...
    if (lineIndex >= lineLen)
   	style = FILL_MASK;
    	...
    	style = (unsigned char)_BufGetCharacter(styleBuf, pos);
...
static int stringWidth(textDisp *textD, char *string, int length, int style)
{
    ...
    if (style & STYLE_LOOKUP_MASK)
    	fs = textD->styleTable[(style & STYLE_LOOKUP_MASK) - textD->unfinishedStyle].font;
    else 
    	fs = textD->fontStruct;
...
static int measureVisLine(textDisp *textD, int visLineNum)
{
	...
	for (i=0; i<lineLen; i++) {
    	...
    	for (i=0; i<lineLen; i++) {
    	    len = BufGetExpandedChar(textD->buffer, lineStartPos+i,
    		    charCount, expandedChar);
    	    style = (unsigned char)_BufGetCharacter(textD->styleBuffer, 
    			lineStartPos+i);
#if 0
#ifdef PURIFY
    	    purify_printf_with_call_chain("visLineNum: %d  lineStartPos: %d  i: %d  style: %d\n", 
	    		visLineNum, lineStartPos, i, style);
#endif
#endif
    		if (style == textD->unfinishedStyle) {
    	    	/* encountered "unfinished" style, trigger parsing */
    	    	(textD->unfinishedHighlightCB)(textD, lineStartPos+i, textD->highlightCBArg);
    	    	style = (unsigned char)_BufGetCharacter(textD->styleBuffer, 
    				lineStartPos+i);
#if 1
#ifdef PURIFY
    	    ...
    	    width += XTextWidth(textD->styleTable[style-textD->unfinishedStyle].font, expandedChar,
    	    	    len);
    	    charCount += len;
	}
    }
...
static int visLineLength(textDisp *textD, int visLineNum)
{
    ...
    if (lineStartPos == -1 || lineStartPos > textD->buffer->length)
    	return 0;
