...
by pressing escape.  For example, to find the string \"thing\" type:\n\
\n\
    <ctrl-r>thing<return>\n\
\n\
...
press the \"Replace In Selection\" button in the dialog.  Note that selecting text \
...
and drag it to the other end.  The text will become highlighted.  This is called \
the primary selection.  To \
...
After releasing the mouse button, you can still adjust the selection \
by clicking the right mouse button where you want the selection adjusted to \
or by holding the right mouse button and dragging on either end of the \
selection. \
...
\n\
If the shift key is depressed while performing any of the above selection \
operations \
you will be able to create an additional selection (called the secondary \
selection). The secondary selection is shown visually by an underline under \
the text that is selected. As soon as the shift key is released, the contents \
of this selection will be \
...
shaped cursor when the mouse is outside of the destination window.  \
If there is a (primary) selection, adjacent to the cursor \
in the destination window, the new text will replace the selected text. \
\n\
The middle mouse button can also be used to create a secondary selection \
by dragging it over the text to be selected. \
As soon as the button is released, the selection will be \
copied just as it is for the shift key. \
If the shift key is depressed when the selection is started \
the selection will be moved rather than copyied. This will delete\
...
combination.\n\
\n\
For the users who want an improved secondary selection model add the \
...
nedit*NEditText.translations: #override \\n\\\n\
            ...
            <KeyPress>osfCopy: \\n\\\n\
            <KeyPress>osfPaste: \\n\\\n\
            <KeyRelease>osfCut: secondary-or-clipboard(move, cut)\\n\\\n\
            <KeyRelease>osfCopy: secondary-or-clipboard(exchange, copy)\\n\\\n\
            <KeyRelease>osfPaste: secondary-or-clipboard(copy, paste)\\n\\\n\
            <KeyRelease>osfPrimaryPaste: copy-primary()\\n\n\
...
\n\
These translation are a combination of xterm and textedit.  The left \
mouse button is used to start a selection and the right mouse button is \
used to extend the selection just like in xterm. Shift plus the right \
mouse button will pop up a menu to do Undo/Redo/Cut/Copy/Paste.  \
Note, the bgMenuButton resource also controls this function. Control plus \
shift and the right mouse button is used to scroll the window with the \
mouse.  The \
secondary selection is controlled by holding down either the Copy, Cut, \
or Paste keys just like in textedit. When the key is released the \
...
\n\
   ...
   Paste - The secondary selection will be inserted at the destination\n\
           cursor.\n\
\n\
Be aware that the Copy, Cut, and Paste keys must be setup \
as Mod5 modifier keys. Run the following command to display \
the keys that are setup for Mod5.\n\
\n\
  xmodmap -pm\n\
\n\
If your Copy, Cut, or Paste keys are not listed then execute the \
following to add them.\n\
\n\
  xmodmap -e 'add mod5 = <keysym> <keysym> ...'\n\
\n\
For example, the following applies for Sun workstations:\n\
\n\
  xmodmap -e 'add mod5 = F16 F18 F20'\n\
\n\
To set these up permanently I recommend adding the above command to your \
corresponding .xinitrc file.\n\
""\
\n\
These translation also provide the ability to drag the primary \
selection with the left mouse button just like textedit. \n\
\n\
If you would also like to use the Shift key to control the secondary \
selection then use the following translation table instead.\n\
\n\
nedit*NEditText.translations: #override \\n\\\n\
  Alt        <KeyRelease>Shift_L: exchange()\\n\n\
  Meta       <KeyRelease>Shift_L: exchange()\\n\n\
  Alt        <KeyRelease>Shift_R: exchange()\\n\n\
  Meta       <KeyRelease>Shift_R: exchange()\\n\n\
             <KeyRelease>Shift_L: copy-secondary()\\n\\\n\
             <KeyRelease>Shift_R: copy-secondary()\\n\\\n\
             <KeyPress>osfCut: \\n\\\n\
             <KeyPress>osfCopy: \\n\\\n\
             <KeyPress>osfPaste: \\n\\\n\
             <KeyRelease>osfCut: secondary-or-clipboard(move, cut)\\n\\\n\
  ...
  Mod5 ~Ctrl <Btn1Down>: grab-focus(secondary)\\n\\\n\
  Mod5  Ctrl <Btn1Down>: grab-focus(secondary, rect)\\n\\\n\
  Shift~Ctrl <Btn1Down>: grab-focus(secondary)\\n\\\n\
  Shift Ctrl <Btn1Down>: grab-focus(secondary, rect)\\n\\\n\
 ~Shift Ctrl <Btn1Down>: grab-focus(rect)\\n\\\n\
             <Btn1Down>: drag-or-grab-focus()\\n\\\n\
  Mod5 ~Ctrl  Button1<MotionNotify>: extend-adjust(secondary)\\n\\\n\
  Mod5  Ctrl  Button1<MotionNotify>: extend-adjust(rect, secondary)\\n\\\n\
  Shift~Ctrl  Button1<MotionNotify>: extend-adjust(secondary)\\n\\\n\
  Shift Ctrl  Button1<MotionNotify>: extend-adjust(rect, secondary)\\n\\\n\
 ~Shift Ctrl  Button1<MotionNotify>: extend-adjust(rect)\\n\\\n\
              Button1<MotionNotify>: drag-or-extend-adjust()\\n\\\n\
  Mod5 ~Ctrl <Btn1Up>: extend-end(secondary)\\n\\\n\
  Mod5  Ctrl <Btn1Up>: extend-end(secondary, rect)\\n\\\n\
  Shift~Ctrl <Btn1Up>: extend-end(secondary)\\n\\\n\
  Shift Ctrl <Btn1Up>: extend-end(secondary, rect)\\n\\\n\
 ~Shift Ctrl <Btn1Up>: extend-end(rect)\\n\\\n\
             <Btn1Up>: drag-or-extend-end()\\n\\\n\
""\
  Alt  Shift Ctrl <Btn3Down>: mouse_pan()\\n\n\
  Meta Shift Ctrl <Btn3Down>: mouse_pan()\\n\n\
  Alt  Shift      <Btn3Down>: post_window_bg_menu(rect)\\n\n\
  Meta Shift      <Btn3Down>: post_window_bg_menu(rect)\\n\n\
  Mod5 ~Ctrl <Btn3Down>: extend-start(secondary)\\n\\\n\
  Mod5  Ctrl <Btn3Down>: extend-start(secondary, rect)\\n\\\n\
  Shift~Ctrl <Btn3Down>: extend-start(secondary)\\n\\\n\
  Shift Ctrl <Btn3Down>: extend-start(secondary, rect)\\n\\\n\
 ~Shift Ctrl <Btn3Down>: extend-start(rect)\\n\\\n\
             <Btn3Down>: extend-start()\\n\\\n\
  Alt  Shift Ctrl  Button3<MotionNotify>: mouse_pan()\\n\n\
  Meta Shift Ctrl  Button3<MotionNotify>: mouse_pan()\\n\n\
  Mod5 ~Ctrl  Button3<MotionNotify>: extend-adjust(secondary)\\n\\\n\
  Mod5  Ctrl  Button3<MotionNotify>: extend-adjust(rect, secondary)\\n\\\n\
  Shift~Ctrl  Button3<MotionNotify>: extend-adjust(secondary)\\n\\\n\
  Shift Ctrl  Button3<MotionNotify>: extend-adjust(rect, secondary)\\n\\\n\
 ~Shift Ctrl  Button3<MotionNotify>: extend-adjust(rect)\\n\\\n\
              Button3<MotionNotify>: extend-adjust()\\n\\\n\
  Alt  Shift Ctrl <Btn3Up>: end_drag()\\n\n\
  Meta Shift Ctrl <Btn3Up>: end_drag()\\n\n\
  Mod5 ~Ctrl <Btn3Up>: extend-end(secondary)\\n\\\n\
  Mod5  Ctrl <Btn3Up>: extend-end(secondary, rect)\\n\\\n\
  Shift~Ctrl <Btn3Up>: extend-end(secondary)\\n\\\n\
  Shift Ctrl <Btn3Up>: extend-end(secondary, rect)\\n\\\n\
 ~Shift Ctrl <Btn3Up>: extend-end(rect)\\n\\\n\
             <Btn3Up>: extend-end()\n\
\n\
nedit.bgMenuButton: Shift Ctrl Alt<Btn3Down>\n\
\n\
",
...
file that you are editing, but with the character \"~\" (tilde) on Unix \
...
\n\
			...
			- The checking interval in milliseconds is set by the \n\
			  ...
			  is still supported. \n\
\n\
The options below operate only \
on the current file in the current window, and can not be \
used as default preferences:\n\
\n\
...
substitute the entire string that was matched in the Find operation.  \
...
argument.\n\
\n\
Multiple nedit servers can be started and accessed by using the -sn \
option to specify a unique name for each server. \
\n\
Communication between nc and nedit is through the X window display. \
So as long as X \
windows is set up and working properly, nc will will work \
properly as well.  nc searches for an nedit server running on the \
display specified by the DISPLAY environment variable. It then searches \
for a server started by your user name with the name specified \
by the -sn option if supplied. nc can be used from different machines \
that share a common file system as long as the DISPLAY environment variable \
is set to the same X window display. Be aware that the file access \
is still performed by the machine where the nedit server was started.\n\
\n\
...
""\
    new()                     find()\n\
    open()                    find_dialog()\n\
    open_dialog()             find_incremental\n\
    open_selected()           find_again()\n\
    close()                   find_selection()\n\
    save()                    replace()\n\
    save_as()                 replace_dialog()\n\
    save_as_dialog()          replace_all()\n\
    revert_to_saved()         replace_in_selection()\n\
    include_file()            replace_again()\n\
    include_file_dialog ()    replace_find()\n\
    load_tags_file()          replace_find_again()\n\
    load_tags_file_dialog()   goto_line_number()\n\
    load_macro_file()         goto_line_number_dialog()\n\
    load_macro_file_dialog()  goto_selected()\n\
    print()                   mark()\n\
    print_selection()         mark_dialog()\n\
    exit()                    goto_mark()\n\
                              goto_mark_dialog()\n\
                              select_matching()\n\
                              goto_matching()\n\
    Edit Menu                 find_definition()\n\
    ---------------------     \n\
    undo()                    Shell Menu\n\
    redo()                    -----------------------\n\
    cut_clipboard()           execute_command()\n\
    copy_clipboard()          execute_command_dialog()\n\
    paste_clipboard()         execute_selection_or_line()\n\
    delete()                  filter_selection()\n\
    select_all()              filter_selection_dialog()\n\
    shift_left()              shell_menu_command()\n\
    shift_left_by_tab()       \n\
    shift_right()             Macro Menu\n\
    shift_right_by_tab()      -----------------------\n\
    uppercase()               macro_menu_command()\n\
...
\n\
copy-secondary()\n\
Copies the secondary selection into the primary selection. If \
there is no primary selection the secondary selection is inserted \
at the destination cursor in the window where the mouse was last clicked. \
\n\
copy-to()\n\
...
\n\
drag-or-grab-focus()\n\
An extention of the grab-focus action that supports dragging of the \
primary selection. It prepares for a drag operation if the file is \
writable and the multi-click time has expired and the button press \
occured within the primary selection and the \"secondary\" argument is \
not supplied. If any of the above is not true then the event is passed \
on to the grab-focus action. This action should be attached to the \
button down event. drag-or-extend-adjust and drag-or-extend-end should \
be attached to the button motion notify and button down events \
respectively. \
\n\
drag-or-extend-adjust()\n\
An extention of the extend-adjust action that supports dragging of the \
primary selection. This action should be attached to the button motion \
notify event. \
\n\
drag-or-extend-end()\n\
An extention of the extend-adjust action that supports dragging of the \
primary selection. This action should be attached to the button down \
event. \
\n\
end-of-file()\n\
...
If the user is dragging a block of \
text (primary selection), completes the drag operation and deletes the \
""\
text from it's current location. If the user is performing a mouse pan, \
completes the mouse pan operation.\n\
\n\
end-of-selection()\n\
...
If a secondary selection exists perform the exchange() action \
otherwise perform the copy-clipboard() action. \
\n\
extend-adjust()\n\
...
\n\
move-secondary()\n\
Moves the secondary selection into the primary selection. The \
secondary selection is removed from its original location. If \
there is no primary selection the secondary selection is inserted \
at the destination cursor in the window where the mouse was last clicked. \
\n\
move_to()\n\
...
This action will perform the secondaryAction if a secondary selection \
exists otherwise it will perform the clipboardAction.\n\
""\
\n\
...
static Widget createHelpPanel(Widget parent, int topic)
{
    ...
    HelpTextPanes[topic] = XtVaCreateManagedWidget("helpText",
		...
		NULL);
    XtVaSetValues(sw, XmNworkWindow, HelpTextPanes[topic], NULL);
...
{    
	...
	RaiseShellWindow(HelpWindows[topic]);
    BufSelect(TextGetBuffer(HelpTextPanes[topic]), beginMatch, endMatch, CHAR_SELECT);
