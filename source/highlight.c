...
#ifdef PURIFY
# define _BufGetCharacter BufGetCharacter
#endif
...
/* The number of characters that have to be inserted by the 
** SyntaxHighlightModifyCB to cause the the watch cursor to be displayed. */
#define DISPLAY_WATCH_CURSOR_LIMIT 10000
...
typedef struct _windowHighlightData {
...
{    
    ...
    if (nInserted > 0) {
#ifdef HAVE_ALLOCA
    	insStyle = alloca(sizeof(char) * (nInserted + 1));
#else
    	insStyle = XtMalloc(sizeof(char) * (nInserted + 1));
#endif
    	memset(insStyle, UNFINISHED_STYLE, nInserted);
    ...
    BufSelect(highlightData->styleBuffer, pos, pos+nInserted, CHAR_SELECT);
    
    /* Re-parse around the changed region */
    if (highlightData->pass1Patterns) {
		/* Display the watch cursor if there are a lot of changes */
		if(nInserted >= DISPLAY_WATCH_CURSOR_LIMIT) {
    		BeginWait(window->shell);
		}
    	incrementalReparse(highlightData, window->editorInfo->buffer, pos, nInserted,
...
void StartHighlighting(WindowInfo *window, int warn)
{
    ...
    /* Turn off highlighting before turning it on. This way
	** if we fail the highlight syntax is sure to be off. */
    ...
    stylePtr = styleString = XtMalloc(window->editorInfo->buffer->length + 1);
    if (highlightData->pass1Patterns == NULL) {
    	memset(stylePtr, UNFINISHED_STYLE, window->editorInfo->buffer->length);
    	stylePtr += window->editorInfo->buffer->length;
    } else {
	stringPtr = bufString = BufGetAll(window->editorInfo->buffer);
	parseString(highlightData->pass1Patterns, &stringPtr, &stylePtr,
    		window->editorInfo->buffer->length, &isBOL, &isBOW, False,
    		GetWindowDelimiters(window));
	XtFree(bufString);
    }
    *stylePtr = '\0';
    ...
    XmToggleButtonSetState(window->highlightItem, True, FALSE);
...
/*
** Turn off syntax highlighting and XtFree style buffer, compiled patterns, and
** related data.
*/
void StopHighlighting(WindowInfo *window)
{
    ...
    XmToggleButtonSetState(window->highlightItem, False, FALSE);
...
void printPatterns(highlightDataRec *pass1Patterns, highlightDataRec *pass2Patterns) {
	int i;
	highlightDataRec *hdrPtr;

	for(i = 0, hdrPtr = pass1Patterns; hdrPtr->style != 0; i++, hdrPtr++) {
		printf("pass1Patterns[i].style = %c(%d)\n", hdrPtr->style, hdrPtr->style);
	}
	printf("\n");
	for(i = 0, hdrPtr = pass2Patterns; hdrPtr->style != 0; i++, hdrPtr++) {
		printf("pass2Patterns[i].style = %c(%d)\n", hdrPtr->style, hdrPtr->style);
	}
}
...
** are encountered, warns user with a dialog and returns NULL.  To XtFree the
...
static void freePatterns(highlightDataRec *patterns)
{
    ...
    for (hdrPtr = patterns; hdrPtr->style!=0; hdrPtr++) {
    	if (hdrPtr->startRE != NULL)
    	    XtFree((char *)hdrPtr->startRE);
    	if (hdrPtr->endRE != NULL)
    	    XtFree((char *)hdrPtr->endRE);
    	if (hdrPtr->errorRE != NULL)
    	    XtFree((char *)hdrPtr->errorRE);
    	if (hdrPtr->subPatternRE != NULL)
    	    XtFree((char *)hdrPtr->subPatternRE);
    }
    for (hdrPtr = patterns; hdrPtr->style!=0; hdrPtr++)
    	if (hdrPtr->subPatterns != NULL)
    	    XtFree((char *)hdrPtr->subPatterns);
...
static void handleUnparsedRegionCB(textDisp *textD, int pos, void *cbArg)
{
    ...
    if (pass2Patterns == NULL) {
		...
		purify_printf_with_call_chain(
			"handleUnparsedRegionCB(pass2Patterns == NULL): pos(%d)\n", 
			pos
		);
#endif    
    	return;
    ...
    firstPass2Style = pass2Patterns[1].style;
    	...
    	textBuffer *buf, int pos, int nInserted, char *delimiters)
{
	...
	styleBuf = highlightData->styleBuffer;
	pass1Patterns = highlightData->pass1Patterns;
	pass2Patterns = highlightData->pass2Patterns;
	context = &highlightData->contextRequirements;
	majorStyles = highlightData->majorStyles;
    ...
    if (posInStyle) {
    	for (nPasses=0; ; nPasses++) {
    	    ...
    	    endAt = parseBufferRange(patternOfStyle(pass1Patterns,
    	    	    majorStyles[_BufGetCharacter(styleBuf, beginParse)-UNFINISHED_STYLE]),
        ...
        char *delimiters)
{
    ...
    if (CAN_CROSS_LINE_BOUNDARIES(contextRequirements)) {
    	beginSafety = backwardOneContext(buf, contextRequirements, beginParse);
     	for (p=beginParse; p>beginSafety; p--) {
    	    style = _BufGetCharacter(styleBuf, p-1);
    	...
    	for (beginSafety=max(0,beginParse-1); beginSafety>0; beginSafety--) {
    	    style = _BufGetCharacter(styleBuf, beginSafety);
    	    if (!EQUIVALENT_STYLE(style, beginStyle, firstPass2Style) ||
    	    	    _BufGetCharacter(buf, beginSafety) == '\n') {
    	    	beginSafety++;
    	    	break;
    	    }
    	}
    ...
    else if (endParse>=buf->length || (_BufGetCharacter(buf,endParse-1)=='\n'))
    	endSafety = endParse;
    else
...
{
	int i;
	char *stringPtr, *stylePtr, *startingStringPtr = 0;
	signed char *subExpr;
	highlightDataRec *subPat = 0, *subSubPat;
...
#if 0
/* Causes crash when resizing */
		/* Refresh the display each pass so it doesn't seem like we
		** are completely hung. This may cause problems when reloading
		** the file and syntax highlighting is not turned off. */
		UpdateDisplay();
#endif
		...
		if (pattern->endRE != NULL && ExecRE(pattern->endRE, stringPtr,
			...
			fillStyleString(&stringPtr, &stylePtr, pattern->endRE->endp[0],
				pattern->style, delimiters, isBOL, isBOW);
			/* Process color-only sub-patterns based on the end pattern */
			for (i=0; i<pattern->nSubPatterns; i++) {
				subSubPat = pattern->subPatterns[i];
		...
		/* Parse the remainder of the sub-pattern */	    
			...
			parseString(subPat, &stringPtr, &stylePtr,
				length - (stringPtr - *string), isBOL, isBOW, False,
				delimiters);
		}
    	...
    	char *delimiters)
{
    ...
    char *stylePtr, *stringPtr, temp, *parseStart = string, *parseEnd, *s, *c;
    	...
    	char style, char *delimiters, int *isBOL, int *isBOW)
{
    ...
    if (*stringPtr >= toPtr) {
/* this does happen in parseString() after the if(!anchored) test. So I guess it
** is not a problem so lets not complain about it. */
#ifdef PURIFY
    	if(*stringPtr > toPtr) {
			purify_printf_with_call_chain(
				"fillStyleString(*stringPtr > toPtr): *stringPtr(%d) toPtr(%d)\n", 
				*stringPtr, toPtr
			);
    	}
#endif
    ...
    len = toPtr - *stringPtr;
    memset(*stylePtr, style, len);
	
    *stringPtr += len;
    *stylePtr += len;
    
    if (isBOL != NULL) *isBOL = ((*stringPtr)[-1] == '\n');
    if (isBOW != NULL) *isBOW = isDelim((*stringPtr)[-1], delimiters);
}
    	...
    	int startPos, int endPos, int firstPass2Style)
{
    ...
    for (c=styleString, pos=startPos; pos<modStart && pos<endPos; c++, pos++) {
    	bufChar = _BufGetCharacter(styleBuf, pos);
    ...
    BufSelect(styleBuf, min(modStart, minPos), max(modEnd, maxPos), CHAR_SELECT);
}
...
static void posBeginsLineOrWord(textBuffer *buf, int pos, char *delimiters,
	int *bol, int *bow)
{
    *bol = pos == 0 ? True : (_BufGetCharacter(buf, pos-1) == '\n');
    *bow = pos == 0 ? True : isDelim(_BufGetCharacter(buf, pos-1), delimiters);
}
...
** will satisfy context guarantees for pattern matching for modifications at
    	...
    	reparseContext *context, char *majorStyles, int *pos)
{
    ...
    /* Back up far enough to ensure that expressions can match any characters
       within the pattern's guaranteed context back from the change */
    *pos = backwardOneContext(buf, context, *pos);
    ...
    if (*pos == 0)
    	return True;
    overlapStyle = _BufGetCharacter(styleBuf, *pos);
    ...
    ** from the beginning and end of the style to guarantee that it's not
    ** within the start or end expression match.
    */   
    checkBackTo = 0;
    maxSafeParseStart = backwardOneContext(buf, context, *pos);
    safeParseStart = 0;
    overlapMajorStyle = majorStyles[overlapStyle-UNFINISHED_STYLE];
    for (i = *pos-1; ; i--) {
    	...
    	style = _BufGetCharacter(styleBuf, i);
    	if (overlapMajorStyle != majorStyles[style-UNFINISHED_STYLE]) {
    	    *pos = i + 1;
    	    return True;
    	}
    	...
    	char *styleString)
{
	...
	if(stringPtr < string) {
		purify_printf_with_call_chain(
			"recolorSubexpr(stringPtr < string): stringPtr(%d) string(%d) subexpr(%d) styleString(%d)\n", 
			stringPtr, string, subexpr, styleString
		);
		return;
	}
#endif
