...
static BuiltInSubr SpecialVars[N_SPECIAL_VARS] = {cursorMV, fileNameMV,
	...
	emTabDistMV, useTabsMV, languageModeMV};
static char *SpecialVarNames[N_SPECIAL_VARS] = {"$cursor", "$file_name",
	...
	"$em_tab_dist", "$use_tabs", "$language_mode"};
...
void RegisterMacroSubroutines(void)
{
    ...
    for (i=0; i<N_RETURN_GLOBALS; i++)
    	ReturnGlobals[i] = InstallSymbol(ReturnGlobalNames[i], GLOBAL_SYM,
    	    	noValue);
#ifdef USE_ENVIRONMENT_AS_MACRO_GLOBALS
	{
	char **ep;
	for(ep = environ; *ep; ep++) {
		char *buf = XtMalloc((strlen(*ep)+2)*sizeof(char)); /* '$' and \0 */
		char *var, *val;
		DataValue value;
		buf[0] = '$'; /* Global variable symbols start with a '$' */
		strcpy(buf+1, *ep);
		var = buf;
		val = strchr(var, '=');
		*val = 0;
		...
		value.val.str = AllocString(strlen(val) + 1);
		strcpy(value.val.str, val);
		InstallSymbol(var, GLOBAL_SYM, value);
		XtFree(buf);
	}
	}
#endif
...
static void finishMacroCmdExecution(WindowInfo *window)
{
    ...
    if (closeOnCompletion && !window->editorInfo->filenameSet && !window->editorInfo->fileChanged)
...
static int doRepeatDialogAction(repeatDialog *rd, XEvent *event)
{
	...
	if (!rd->forWindow->editorInfo->buffer->primary.selected) {
      ...
      DataValue *result, char **errMsg)
{
    ...
    if (!readStringArg(argList[0], &string, stringStorage, errMsg))
		...
		for (w=WindowList; w != NULL; w = w->next) {
			sprintf(fullname, "%s%s", w->editorInfo->path, w->editorInfo->filename);
			if (!FILECOMPARE(string, fullname))
				break;
		}
    }
    ...
    result->tag = STRING_TAG;
    result->val.str = AllocString(strlen(w->editorInfo->path)+strlen(w->editorInfo->filename)+1);
    sprintf(result->val.str, "%s%s", w->editorInfo->path, w->editorInfo->filename);
    return True;
}
    	...
    	DataValue *result, char **errMsg)
{
    ...
    if (!BufSubstituteNullChars(string, strlen(string), window->editorInfo->buffer)) {
    	...
    	DataValue *result, char **errMsg)
{
	...
	*errMsg = "Too much binary data in file";
    ...
    /* Do the replace */
    BufReplaceSelected(window->editorInfo->buffer, string, True);
    result->tag = NO_TAG;
    return True;
}
    	...
    	DataValue *result, char **errMsg)
{
    ...
    for (i=0; i<length; i++)
    	result->val.str[i] = toupper(string[i]);
    	...
    	DataValue *result, char **errMsg)
{
    ...
    result->val.str = AllocString(length + 1);
    for (i=0; i<length; i++)
    	result->val.str[i] = tolower(string[i]);
    result->val.str[length] = '\0';
    return True;
}
    	...
    	DataValue *result, char **errMsg)
{
    ...
    newArgList[0].val.str = BufGetAll(window->editorInfo->buffer);
    	...
    	DataValue *result, char **errMsg)
{
    ...
    /* Make the selection */
    BufSelect(window->editorInfo->buffer, start, end, CHAR_SELECT);
    result->tag = NO_TAG;
    return True;
}
    	...
    	DataValue *result, char **errMsg)
{
    ...
    /* Make the selection */
    BufRectSelect(window->editorInfo->buffer, start, end, left, right);
    result->tag = NO_TAG;
    return True;
}
...
{
    result->tag = STRING_TAG;
    result->val.str = AllocString(strlen(window->editorInfo->filename) + 1);
    strcpy(result->val.str, window->editorInfo->filename);
    return True;
}
...
{
    result->tag = STRING_TAG;
    result->val.str = AllocString(strlen(window->editorInfo->path) + 1);
    strcpy(result->val.str, window->editorInfo->path);
    return True;
}
...
{
    result->tag = INT_TAG;
    result->val.n = window->editorInfo->buffer->length;
    return True;
}
...
{
    result->tag = INT_TAG;
    result->val.n = window->editorInfo->buffer->primary.selected ?
    	    window->editorInfo->buffer->primary.start : -1;
    return True;
}
    	...
    	DataValue *result, char **errMsg)
{
    result->tag = INT_TAG;
    result->val.n = window->editorInfo->buffer->primary.selected ?
    	    window->editorInfo->buffer->primary.end : -1;
    return True;
}
...
static int selectionLeftMV(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg)
{
    selection *sel = &window->editorInfo->buffer->primary;
...
{
    result->tag = INT_TAG;
    result->val.n = window->editorInfo->buffer->tabDist;
    return True;
}
...
{
    result->tag = INT_TAG;
    result->val.n = window->editorInfo->buffer->useTabs;
    return True;
}
