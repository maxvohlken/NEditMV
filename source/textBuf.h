...
typedef enum _selType {CHAR_SELECT, WORD_SELECT, LINE_SELECT} selType;
...
/* Faster version of BufGetCharacter().
** Used to get a character from the buffer without bounds checking. */
#define _BufGetCharacter(buffer, pos) \
	(unsigned char)(((pos) < (buffer)->gapStart) ? (buffer)->buf[(pos)] : \
		((buffer)->buf[(pos) + (buffer)->gapEnd - (buffer)->gapStart]))

#if 0
#define BufGetCharacter(buf, pos) _BufGetCharacter(buf, pos)
#endif

#define BufGetLength(buffer) (buffer)->length
