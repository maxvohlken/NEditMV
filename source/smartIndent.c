...
#define N_DEFAULT_INDENT_SPECS 2
static smartIndentRec DefaultIndentSpecs[N_DEFAULT_INDENT_SPECS] = {{"C",
...
Preferences -> Language Mode.", "Dismiss", modeName);
    ...
    window->editorInfo->smartIndentData = (void *)winData;
}

void EndSmartIndent(WindowInfo *window)
{
    windowSmartIndentData *winData =
    	    (windowSmartIndentData *)window->editorInfo->smartIndentData;
...
void SmartIndentCB(Widget w, XtPointer clientData, XtPointer callData) 
{
    ...
    if (window->editorInfo->smartIndentData == NULL)
    	return;
