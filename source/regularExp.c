...
regexp *CompileRE(char *exp, char **errorText)
{
    ...
    *errorpointer = NULL;
    ...
    /* Allocate space. */
    r = (regexp *)XtMalloc(sizeof(regexp) + (unsigned)regsize);
    if (r == NULL)
	FAIL("out of space");
	...
	int isbow, char *delimiters)
{
	...
	for (s=end-1; s>=string; s--)
	    if (*s == '\n')
	    	if (regtry(prog, s+1) && prog->endp[0] <= end)
	    	    return(1);
	if (regtry(prog, string) && prog->endp[0] <= end)
	    return(1);
	return(0);
    }
	...
	for (s=end; s>=string; s--)
	    if (*s == prog->regstart)
	    	if (regtry(prog, s) && prog->endp[0] <= end)
	    	    return(1);
	return(0);
    }
