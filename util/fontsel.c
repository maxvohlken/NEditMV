...
char 	*FontSel(Widget parent, int showPropFonts, char *currFont, char *title)
{
	...
	title = title ? title : "Font Selector";

	n = 0;
	XtSetArg(args[n], XmNtitle, title); n++;
	dialog	= XmCreateDialogShell (parent, "fontSelectorDialog", args, n);
	...
	while (! ctrlBlk.exitFlag) {
   		XEvent event;
   		XtAppNextEvent(XtWidgetToApplicationContext(form), &event);
