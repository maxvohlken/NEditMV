...
void RestorePreferences(XrmDatabase prefDB, XrmDatabase appDB,
	char *appNameIn, char *appClass, PrefDescripRec *rsrcDescrip, int nRsrc)
{
    char rsrcName[256], rsrcClass[256], *valueString, *type;
    XrmValue rsrcValue;
    int i;
	char *appName = XtNewString(appNameIn);
	
#ifdef _WIN32
	/* Remove the .exe from appName for resource lookups. */
	{
	...
	_splitpath(appName, drive, dir, fname, ext);
	_makepath(appName, NULL, NULL, fname, NULL);
	}
#endif
