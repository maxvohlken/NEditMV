 ...
 *    char *     initial_value - if dialog type = DF_PROMPT2, then: *
 *                        A string that contains the initial value for *
 *                        the text field.
 ...
 *	but_pressed = DialogF (DF_PROMPT2, toplevel, 0, categories[i], "New %s", 	      *
 *				new_sub_category, categories[i]);  	      *
*/
		...
		char *msgstr, ...)		/* variable # arguments */
{
	...
	if ((dialog_num > (sizeof(dialog_types)/sizeof(dialog_types[0])))
	|| (dialog_num <= 0)) {
		printf ("\nError calling DialogF - Unsupported dialog type\n");
		return (0);
	}
	prompt = (dialog_type == DF_PROMPT || dialog_type == DF_PROMPT2);
    ...
    if (dialog_type == DF_PROMPT2) {
		initial_value = va_arg(var, char*);
    }
    ...
    */
    ...
    if (prompt) {				/* Prompt dialog */
    	if(initial_value) {
			textstr_xms = XmStringCreateLtoR (initial_value, XmSTRING_DEFAULT_CHARSET);
			XtSetArg (args[argcount], XmNtextString, textstr_xms);
				argcount++;
		}
		...
		XtSetArg (args[argcount], XmNdialogStyle, XmDIALOG_APPLICATION_MODAL);
			argcount ++;
#else
		if(dialog_type == DF_INF) {
			XtSetArg (args[argcount], XmNdialogStyle, XmDIALOG_PRIMARY_APPLICATION_MODAL);
				argcount ++;
		} else {
