...
/*									   */
int GetExistingFilename (Widget parent, char *promptString, char *filename, char *title) 
{
    ...
    titleString = XmStringCreateSimple((title == 0 || *title == 0) ? " " : title);
...
int HandleCustomExistFileSB(Widget existFileSB, char *filename)
{
    ...
    while (!done_with_dialog) {
   		XEvent event;
   		XtAppNextEvent(XtWidgetToApplicationContext(existFileSB), &event);
...
int HandleCustomNewFileSB(Widget newFileSB, char *filename)
{
...
#endif
#endif
    ManageDialogCentered(newFileSB);
    ...
    while (!done_with_dialog) {
   		XEvent event;
   		XtAppNextEvent(XtWidgetToApplicationContext(newFileSB), &event);
   		DISPATCH_EVENT(&event);
    }
...
static int doYesNoDialog(char *filename)
{
    ...
    ManageDialogCentered(YesNoDialog);

    while (YesNoResult == ynNone) {
    	XEvent event;
    	XtAppNextEvent(XtWidgetToApplicationContext(YesNoDialog), &event);
    	DISPATCH_EVENT(&event);
    }
...
static void doErrorDialog(char *errorString, char *filename)
{
    ...
    XmStringFree(mString);
    ManageDialogCentered(ErrorDialog);

    while (!ErrorDone) {
   		XEvent event;
   		XtAppNextEvent(XtWidgetToApplicationContext(ErrorDialog), &event);
   		DISPATCH_EVENT(&event);
    }
...
static void newHelpCB(Widget w, Widget helpPanel, caddr_t call_data)
{
    ManageDialogCentered(helpPanel);
}
	...
	Boolean *continueDispatch)
{
    ...
    *continueDispatch = False;
    ...
    for (i=0; i<nItems; i++) {
    	...
    	XtFree(itemString);
    	cmp = FILENCOMPARE(name, keystrokes, nKeystrokes);
    	if (cmp == 0) {
    	    selectPos = i+1;
    	    break;
    	} else if (cmp > 0) {
