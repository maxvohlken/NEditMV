...
#define DISPATCH_EVENT(event) XtDispatchEvent(event)
#endif
...
static int WindowCenteredDialogsEnabled = False;
static int DialogsJustOutsideWindowEnabled = True;
...
/*
** ManageDialogCenteredOnWindow is used to position a dialog
** (dialogChild) centered above another window(window). This is
** only needed if window is not the parent of dialogChild. If
** window is the parent of dialogChild then XtManageChild will
** by default place the dialog child centered over the parent window.
*/ 
void ManageDialogCenteredOnWindow(Widget dialogChild, Widget window)
{
    Widget shell = XtParent(dialogChild);
    Window root;
    unsigned int width, height, borderWidth, depth;
	...
	/* Get the location of the upper left corner of the window
	** relative to the root window */
	XtTranslateCoords(window, 0, 0, &x2, &y2);	
	x = x2;
	y = y2;
	
    /* Find the center of the window */
    XGetGeometry(XtDisplay(window), XtWindow(window), &root, &winX, &winY,
    	    &width, &height, &borderWidth, &depth);
    width += 2 * borderWidth;
    height += 2 * borderWidth;
	
	/* make x and y correspond to the center of the window */
    x += width/2;
    y += height/2;

    /* Translate the window position (x, y) into a position for the new
       window that will place the new window in the center of the window
	   specified. */
    XGetGeometry(XtDisplay(shell), XtWindow(shell), &root, &winX, &winY,
    	    &width, &height, &borderWidth, &depth);
    width += 2 * borderWidth;
...
/*
** ManageDialogJustOutsideWindow is used in place of XtManageChild for
** popping up a dialog to enable the dialog to be centered under the
** mouse pointer.  Whether it pops up the dialog centered under the pointer
** or in its default position centered over the parent widget, depends on
** the value set in the SetPointerCenteredDialogs call.
*/ 
void ManageDialogJustOutsideWindow(Widget dialogChild, Widget window)
{
    Widget shell = XtParent(dialogChild);
    Window root;
    unsigned int winW, winH, winBorderW, depth;
    unsigned int dialogW, dialogH, dialogBorderW;
    int winX, winY;
	int dialogX, dialogY;
	int dialogXMin, dialogXMax, dialogYMin, dialogYMax;
	Position posX, posY;
    Boolean mappedWhenManaged;
	Widget winWMShell = FindWMShell(window);
	Widget dialogWMShell;
    int winDecoXW, winDecoYW;

/* 	window = XtParent(dialogChild); */
		
    /* If this feature is not enabled, just manage the dialog */
    if (!DialogsJustOutsideWindowEnabled) {
    	XtManageChild(dialogChild);
    	return;
    }
	...
	dialogWMShell = FindWMShell(dialogChild);

	/* Get the size of the parent window. x and y are relative to parent. */
    XGetGeometry(XtDisplay(winWMShell), XtWindow(winWMShell), &root, &winX, &winY,
    	    &winW, &winH, &winBorderW, &depth);
	
	/* Assume the size of the window manager decoration is the x and y above */
	winDecoXW = winX;
	winDecoYW = winY;
	
	/* Adjust the width and height for the border. */
    winW += 2 * winBorderW;
    winH += 2 * winBorderW;
#ifdef MISC_C_DEBUGGING
printf("winWMShell: winX=%d winY=%d winW=%d winH=%d dialogBorderW=%d\n", winX, winY, winW, winH, winBorderW);
#endif

	/* Get the location of the upper left corner of the window
	** relative to the root window. The dialogs x and y are relative
	** to the root window. */
	XtTranslateCoords(winWMShell, 0, 0, &posX, &posY);
	
	/* Adjust for our location inside our parent(WM Decoration) */
	winX = posX - winDecoXW;
	/* Dont adjust the y for the WM decoration */
	winY = posY /*- winDecoYW*/;
#ifdef MISC_C_DEBUGGING
printf("winWMShell: posX=%d posY=%d\n", posX, posY);
#endif

    /* Get the size of the dialog */
    XGetGeometry(XtDisplay(dialogWMShell), XtWindow(dialogWMShell), 
			&root, &dialogX, &dialogY,
    	    &dialogW, &dialogH, &dialogBorderW, &depth);
    dialogW += 2 * dialogBorderW;
    dialogH += 2 * dialogBorderW;
#ifdef MISC_C_DEBUGGING
printf("dialogWMShell: dialogX=%d dialogY=%d dialogW=%d dialogH=%d dialogBorderW=%d\n", dialogX, dialogY, dialogW, dialogH, dialogBorderW);
#endif
	...
	** relative to the root window. */
	XtTranslateCoords(winWMShell, 0, 0, &posX, &posY);
#ifdef MISC_C_DEBUGGING
printf("dialogWMShell: posX=%d posY=%d\n", posX, posY);
#endif

	/* Adjust for our location inside our parent(WM Decoration) */
	dialogX = posX - dialogX;
	dialogY = posY - dialogY;

	/* Find the min and max values for the x and y of the dialog. */
	dialogXMin = 0 + dialogBorderW;
	dialogYMin = 0 + dialogBorderW;
	dialogXMax = XtScreen(shell)->width - dialogW;
    dialogYMax = XtScreen(shell)->height - dialogH;
	
	/* Try to place the dialog on the right side of the parent 
	** aligned at the top. */
	dialogX = winX + winW + winDecoXW;
	dialogY = winY;

#ifdef MISC_C_DEBUGGING
printf("dialogXMin=%d dialogXMax=%d dialogYMin=%d dialogYMax=%d\n", dialogXMin, dialogXMax, dialogYMin, dialogYMax);
printf("right: dialogX=%d dialogY=%d dialogW=%d dialogH=%d dialogBorderW=%d\n", dialogX, dialogY, dialogW, dialogH, dialogBorderW);
#endif

	/* If there is not enough room try the left side. */
	if(dialogX > dialogXMax) {
		dialogX = winX - dialogW - winDecoXW;
		dialogY = winY;
#ifdef MISC_C_DEBUGGING
printf("left:dialogX=%d dialogY=%d dialogW=%d dialogH=%d\n", dialogX, dialogY, dialogW, dialogH);
#endif
	}
	
	/* If there is not enough room on the left then just place
	** the dialog like normal. */
...
#if 0
    /* Ensure that the dialog remains on screen */
    if (dialogX < dialogXMin) dialogX = dialogXMin;
    if (dialogX > dialogXMax) dialogX = dialogXMax;
    if (dialogY < dialogYMin) dialogY = dialogYMin;
    if (dialogY > dialogYMax) dialogY = dialogYMax;
#endif

    /* Set desired window position in the DialogShell */
    XtVaSetValues(shell, XmNx, dialogX, XmNy, dialogY, NULL);
...
#ifdef MISC_C_DEBUGGING
    /* Get the size of the dialog */
printf("dialogX=%d dialogY=%d dialogW=%d dialogH=%d\n", dialogX, dialogY, dialogW, dialogH);
    XGetGeometry(XtDisplay(shell), XtWindow(shell), 
			&root, &dialogX, &dialogY,
    	    &dialogW, &dialogH, &dialogBorderW, &depth);
    dialogW += 2 * dialogBorderW;
    dialogH += 2 * dialogBorderW;
printf("after: dialogX=%d dialogY=%d dialogW=%d dialogH=%d\n", dialogX, dialogY, dialogW, dialogH);
#endif
}

void ManageDialogCentered(Widget dialogChild)
{
	if (PointerCenteredDialogsEnabled) {
		ManageDialogCenteredOnPointer(dialogChild);
	} else {
    	XtManageChild(dialogChild);
    }
}
...
/*
** Cause dialogs created by libNUtil.a routines (such as DialogF and
** GetNewFilename), and dialogs which use ManageDialogJustOutsideWindow
** to pop up just outside the parent window (state = True), or pop up in 
** their default positions (state = False)
*/
void SetDialogsJustOutsideWindowEnabled(int state)
{
    DialogsJustOutsideWindowEnabled = state;
}
...
/*
** StrCaseCmp compares its arguments IGNORING case differences.
** Returns 0 if the two strings are equal. Otherwise returns -1 
** if str1 is less and str2 and 1 if str1 is greater than str2.
*/
